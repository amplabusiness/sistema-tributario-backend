...
import { XMLParser, XMLDocumentType } from '../../../src/services/parsers/xml-parser';
...
describe('XML Parser', () => {
  ...
  describe('Parse CTe', () => {
    it('deve fazer parsing de CTe válido', async () => {
      ...
      expect(resultado.status).toBe('autorizada');
      expect(resultado.itens).toHaveLength(0); // CTe não tem itens detalhados
    });
  });
  ...
  describe('Validação de XML', () => {
    ...
    it('deve rejeitar tipo de documento não suportado', async () => {
      ...
      await expect(xmlParser.parseXML(xmlContent, 'INVALID' as XMLDocumentType)).rejects.toThrow('Tipo de documento não suportado: INVALID');
    });
  });
