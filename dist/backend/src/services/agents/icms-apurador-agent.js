...
class ICMSApuradorAgent {
    ...
    async apurarICMSAutomatico(empresaId, periodo, documentos) {
        ...
        try {
            (0, logger_1.logInfo)('🚀 AGENTE 3: Iniciando apuração ICMS 100% autônoma', {
                empresaId,
                periodo,
                apuracaoId,
            });
            ...
            (0, logger_1.logInfo)('✅ AGENTE 3: Apuração ICMS concluída com sucesso', {
                apuracaoId,
            ...
            (0, logger_1.logError)('❌ AGENTE 3: Erro na apuração ICMS', error instanceof Error ? error : new Error('Unknown error'));
            return {
                id: apuracaoId,
                empresaId,
                periodo,
    ...
    async extrairRegrasAutomaticamente(empresaId) {
        try {
            (0, logger_1.logInfo)('🔍 AGENTE 3: Extraindo regras automaticamente', { empresaId });
            ...
            (0, logger_1.logInfo)('✅ AGENTE 3: Extração de regras concluída', {
                empresaId,
                regrasExtraidas: this.regras.length,
            });
        }
        catch (error) {
            (0, logger_1.logError)('❌ AGENTE 3: Erro na extração de regras', error instanceof Error ? error : new Error('Unknown error'));
            throw error;
        }
    }
    ...
    `;
        try {
            ...
            (0, logger_1.logError)('❌ AGENTE 3: Erro na análise com IA', error instanceof Error ? error : new Error('Unknown error'));
            return [];
        }
    }
    async validarRegrasAutomaticamente(regras) {
        ...
        for (const regra of regras) {
            if (regra.confianca < this.config.confidenceThreshold) {
                (0, logger_1.logWarn)('⚠️ AGENTE 3: Regra rejeitada por baixa confiança', {
                    regra: regra.nome,
            ...
            if (!this.validarEstruturaRegra(regra)) {
                (0, logger_1.logWarn)('⚠️ AGENTE 3: Regra rejeitada por estrutura inválida', {
                    regra: regra.nome,
                });
                continue;
            }
            if (!this.validarConsistenciaRegra(regra)) {
                (0, logger_1.logWarn)('⚠️ AGENTE 3: Regra rejeitada por inconsistência', {
                    regra: regra.nome,
                });
                continue;
            }
    ...
    async processarDocumentosAutomaticamente(documentos, empresaId) {
        ...
        for (const docId of documentos) {
            try {
                ...
                (0, logger_1.logError)('❌ AGENTE 3: Erro ao processar documento', {
                    docId,
    ...
    `;
        try {
            ...
            (0, logger_1.logError)('❌ AGENTE 3: Erro ao gerar observações', error instanceof Error ? error : new Error('Unknown error'));
        }
        return observacoes;
    }
    ...
    async gerarRelatoriosAutomaticamente(apuracao) {
        (0, logger_1.logInfo)('📊 AGENTE 3: Gerando relatórios automaticamente', { apuracaoId: apuracao.id });
    }
