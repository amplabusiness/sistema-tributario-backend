...
const RATE_LIMIT = {
    requestsPerMinute: parseInt(process.env.OPENAI_RATE_LIMIT) || 60,
...
async function makeOpenAIRequest(prompt, config = {}) {
    ...
    try {
        ...
        (0, logger_1.logInfo)('Requisição OpenAI concluída', {
            model,
            tokens,
            cost: `$${cost.toFixed(4)}`,
        ...
        const duration = Date.now() - startTime;
        (0, logger_1.logError)('Erro na requisição OpenAI', {
