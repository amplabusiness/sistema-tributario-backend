...
class MultiEmpresaWatcher {
    ...
    async start() {
        if (this.isRunning) {
            logger_1.default.warn('MultiEmpresaWatcher já está em execução');
            return;
        }
        this.isRunning = true;
        logger_1.default.info('Iniciando MultiEmpresaWatcher', {
    ...
    async scanDirectory() {
        try {
            if (!(0, fs_1.existsSync)(this.config.basePath)) {
                logger_1.default.warn('Diretório base não existe', { basePath: this.config.basePath });
                return;
            }
            ...
            logger_1.default.info('Varredura de diretório concluída', {
                totalFiles: files.length,
                newFiles: files.filter(f => !this.processedFiles.has(f.path)).length,
            });
        }
        catch (error) {
            logger_1.default.error('Erro durante varredura de diretório', {
    ...
    async discoverFiles(dirPath) {
        ...
        try {
            ...
            logger_1.default.error('Erro ao descobrir arquivos', {
                error: error instanceof Error ? error.message : 'Unknown error',
                dirPath,
            });
        }
    ...
    async analyzeFile(filePath, stats) {
        try {
            ...
            if (stats.size > this.config.maxFileSize) {
                logger_1.default.warn('Arquivo muito grande, ignorando', {
                    filePath,
                    size: stats.size,
            ...
            try {
                ...
                logger_1.default.warn('Não foi possível ler o arquivo para extrair dados da empresa', {
                    filePath,
                    error: error instanceof Error ? error.message : 'Unknown error',
                });
            }
            ...
            logger_1.default.error('Erro ao analisar arquivo', {
                error: error instanceof Error ? error.message : 'Unknown error',
                filePath,
            });
            return null;
    ...
    async processFile(fileInfo) {
        try {
            logger_1.default.info('Processando arquivo', {
            ...
            logger_1.default.info('Arquivo processado com sucesso', {
                filename: fileInfo.filename,
                tipo: fileInfo.tipo,
            });
        }
        catch (error) {
            logger_1.default.error('Erro ao processar arquivo', {
                error: error instanceof Error ? error.message : 'Unknown error',
                filename: fileInfo.filename,
                tipo: fileInfo.tipo,
            });
    ...
    async processProtegeFile(fileInfo) {
        try {
            if (!fileInfo.empresaId) {
                logger_1.default.warn('Arquivo PROTEGE sem empresa identificada', {
                    filename: fileInfo.filename,
                });
                return;
            }
            logger_1.default.info('Processando arquivo PROTEGE', {
                filename: fileInfo.filename,
                empresaId: fileInfo.empresaId,
            });
            const arquivo = {
            ...
            logger_1.default.info('Arquivo PROTEGE processado com sucesso', {
                filename: fileInfo.filename,
                empresaId: fileInfo.empresaId,
            ...
            if (fileInfo.ano && fileInfo.mes) {
                ...
                try {
                    ...
                    logger_1.default.info('PROTEGE calculado automaticamente', {
                        empresaId: fileInfo.empresaId,
                        periodo,
                    ...
                    logger_1.default.warn('Erro ao calcular PROTEGE automaticamente', {
                        empresaId: fileInfo.empresaId,
                        periodo,
                        error: error instanceof Error ? error.message : 'Unknown error',
                    });
            ...
            logger_1.default.error('Erro ao processar arquivo PROTEGE', {
                error: error instanceof Error ? error.message : 'Unknown error',
                filename: fileInfo.filename,
                empresaId: fileInfo.empresaId,
            });
    ...
    async processGenericFile(fileInfo) {
        try {
            ...
            logger_1.default.info('Processando arquivo genérico', documentData);
        }
        catch (error) {
            logger_1.default.error('Erro ao processar arquivo genérico', {
                error: error instanceof Error ? error.message : 'Unknown error',
                filename: fileInfo.filename,
            });
            throw error;
    ...
    async processSpedFile(fileInfo) {
        try {
            logger_1.default.info('Processando arquivo SPED automaticamente', {
                filename: fileInfo.filename,
                empresaId: fileInfo.empresaId,
            });
            const result = await document_processor_1.DocumentProcessor.processSped(fileInfo.path, fileInfo.empresaId);
            logger_1.default.info('Arquivo SPED processado com sucesso', {
                filename: fileInfo.filename,
                tipo: result.tipo,
            ...
            logger_1.default.error('Erro ao processar arquivo SPED', {
                error: error instanceof Error ? error.message : 'Unknown error',
                fileInfo,
            });
        }
    ...
    clearProcessedFiles() {
        ...
        logger_1.default.info('Lista de arquivos processados limpa');
    }
}
