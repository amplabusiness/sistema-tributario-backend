...
class DocumentParser {
    ...
    async parseDocument(filePath) {
        ...
        logger_1.logger.info(`Iniciando parsing do documento: ${fileName}`, { filePath, fileType: fileExtension });
        try {
            ...
            switch (fileExtension) {
                case '.txt':
                    if (fileName.includes('SPED') || fileName.includes('sped')) {
                        ...
                        if (content.includes('|0000|') && content.includes('|C100|')) {
                            parsedData = await this.spedFiscalParser.parse(content);
                            ...
                            parsedData = await this.spedContribuicoesParser.parse(content);
                            companyData = this.extractCompanyDataFromSped(parsedData);
                ...
                case '.xls':
                    if (fileName.toLowerCase().includes('icms') || fileName.toLowerCase().includes('regras')) {
                        parsedData = await this.icmsRulesParser.parse(fileBuffer);
                        companyData = this.extractCompanyDataFromExcel(parsedData);
                    }
                    break;
                case '.pdf':
                    if (fileName.toLowerCase().includes('protege')) {
                        parsedData = await this.protegeParser.parse(fileBuffer);
            ...
            logger_1.logger.info(`Documento parseado com sucesso: ${fileName}`, {
            ...
            logger_1.logger.error(`Erro ao fazer parsing do documento: ${fileName}`, { error, filePath });
            throw error;
        }
    }
