...
router.post('/apurar', auth_1.authenticateToken, [
    ...
    try {
        ...
        (0, logger_1.logInfo)('🚀 API: Iniciando apuração Federal automática', {
            empresaId,
            periodo,
            tipoApuracao,
        ...
        for (const documentId of documentos || []) {
            try {
                ...
                (0, logger_1.logError)('❌ API: Erro ao processar documento Federal', {
                    documentId,
        ...
        (0, logger_1.logInfo)('✅ API: Apuração Federal concluída', {
            empresaId,
            periodo,
        ...
        (0, logger_1.logError)('❌ API: Erro na apuração Federal', error instanceof Error ? error : new Error('Unknown error'));
        res.status(500).json({
            success: false,
...
router.post('/documento/:documentId', auth_1.authenticateToken, [
    ...
    try {
        ...
        (0, logger_1.logInfo)('🚀 API: Processando documento Federal específico', {
            documentId,
            tipoApuracao,
            userId: req.user?.id,
        });
        ...
        const totais = calcularTotaisFederais(resultados);
        (0, logger_1.logInfo)('✅ API: Documento Federal processado', {
            documentId,
            itensProcessados: resultados.length,
            totais,
        });
        ...
        (0, logger_1.logError)('❌ API: Erro ao processar documento Federal', error instanceof Error ? error : new Error('Unknown error'));
        res.status(500).json({
            success: false,
...
router.get('/apuracoes', auth_1.authenticateToken, [
    ...
    try {
        ...
        (0, logger_1.logInfo)('🔍 API: Consultando apurações Federais', {
            empresaId,
            periodo,
            tipoApuracao,
            limit,
        ...
        (0, logger_1.logError)('❌ API: Erro ao consultar apurações Federais', error instanceof Error ? error : new Error('Unknown error'));
        res.status(500).json({
            success: false,
...
router.get('/dashboard', auth_1.authenticateToken, [
    ...
    try {
        ...
        (0, logger_1.logInfo)('📊 API: Gerando dashboard Federal', {
            empresaId,
            periodo,
            userId: req.user?.id,
        });
        ...
        (0, logger_1.logError)('❌ API: Erro ao gerar dashboard Federal', error instanceof Error ? error : new Error('Unknown error'));
        res.status(500).json({
            success: false,
...
router.get('/beneficios', auth_1.authenticateToken, [
    ...
    try {
        ...
        (0, logger_1.logInfo)('🔍 API: Consultando benefícios fiscais', {
            empresaId,
            periodo,
            tipo,
            userId: req.user?.id,
        ...
        (0, logger_1.logError)('❌ API: Erro ao consultar benefícios fiscais', error instanceof Error ? error : new Error('Unknown error'));
        res.status(500).json({
            success: false,
...
router.get('/relatorios/:periodo', auth_1.authenticateToken, [
    ...
    try {
        ...
        (0, logger_1.logInfo)('📄 API: Gerando relatório Federal', {
            empresaId,
            periodo,
            formato,
            tipo,
        ...
        (0, logger_1.logError)('❌ API: Erro ao gerar relatório Federal', error instanceof Error ? error : new Error('Unknown error'));
        res.status(500).json({
            success: false,
