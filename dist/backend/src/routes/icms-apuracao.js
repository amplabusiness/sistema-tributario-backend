...
router.post('/apurar', auth_1.authenticateToken, [
    ...
    try {
        ...
        (0, logger_1.logInfo)('🚀 API: Iniciando apuração ICMS automática', {
            empresaId,
            periodo,
        ...
        (0, logger_1.logInfo)('✅ API: Apuração ICMS concluída', {
        ...
        (0, logger_1.logError)('❌ API: Erro na apuração ICMS', error instanceof Error ? error : new Error('Unknown error'));
        res.status(500).json({
            success: false,
...
router.get('/apuracoes', auth_1.authenticateToken, [
    ...
    try {
        ...
        (0, logger_1.logInfo)('🔍 API: Consultando apurações ICMS', {
            empresaId,
            periodo,
            status,
            limit,
        ...
        (0, logger_1.logError)('❌ API: Erro ao consultar apurações', error instanceof Error ? error : new Error('Unknown error'));
        res.status(500).json({
            success: false,
...
router.get('/apuracoes/:id', auth_1.authenticateToken, [
    ...
    try {
        const { id } = req.params;
        (0, logger_1.logInfo)('🔍 API: Consultando apuração ICMS específica', {
            apuracaoId: id,
            userId: req.user?.id,
        });
        const apuracao = null;
        ...
        (0, logger_1.logError)('❌ API: Erro ao consultar apuração', error instanceof Error ? error : new Error('Unknown error'));
        res.status(500).json({
            success: false,
...
router.get('/regras', auth_1.authenticateToken, [
    ...
    try {
        ...
        (0, logger_1.logInfo)('🔍 API: Consultando regras ICMS', {
            empresaId,
            tipo,
            ativo,
            userId: req.user?.id,
        ...
        (0, logger_1.logError)('❌ API: Erro ao consultar regras', error instanceof Error ? error : new Error('Unknown error'));
        res.status(500).json({
            success: false,
...
router.get('/dashboard', auth_1.authenticateToken, [
    ...
    try {
        ...
        (0, logger_1.logInfo)('📊 API: Gerando dashboard ICMS', {
            empresaId,
            periodo,
            userId: req.user?.id,
        });
        ...
        (0, logger_1.logError)('❌ API: Erro ao gerar dashboard', error instanceof Error ? error : new Error('Unknown error'));
        res.status(500).json({
            success: false,
...
router.post('/regras/extrair', auth_1.authenticateToken, [
    ...
    try {
        ...
        (0, logger_1.logInfo)('🔍 API: Extraindo regras ICMS manualmente', {
            empresaId,
        ...
        (0, logger_1.logError)('❌ API: Erro ao extrair regras', error instanceof Error ? error : new Error('Unknown error'));
        res.status(500).json({
            success: false,
...
router.get('/relatorios/:apuracaoId', auth_1.authenticateToken, [
    ...
    try {
        ...
        (0, logger_1.logInfo)('📄 API: Gerando relatório ICMS', {
            apuracaoId,
            formato,
            userId: req.user?.id,
        });
        ...
        (0, logger_1.logError)('❌ API: Erro ao gerar relatório', error instanceof Error ? error : new Error('Unknown error'));
        res.status(500).json({
            success: false,
