...
router.post('/processar', auth_1.authenticateToken, estoqueValidation, validation_1.validateRequest, async (req, res) => {
    try {
        ...
        (0, logger_1.logInfo)('Processamento de estoque e CIAP iniciado', {
            empresa,
            cnpj,
            periodo,
            userId: req.user?.id
        ...
        (0, logger_1.logError)('Erro no processamento de estoque e CIAP', error instanceof Error ? error : new Error('Unknown error'));
...
router.get('/status/:processamentoId', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        (0, logger_1.logError)('Erro ao buscar status do processamento', error instanceof Error ? error : new Error('Unknown error'));
        res.status(constants_1.HTTP_STATUS.INTERNAL_SERVER_ERROR).json({
            success: false,
            error: constants_1.MESSAGES.ERROR.INTERNAL_ERROR,
            message: 'Erro interno do servidor',
...
router.get('/estoque/:empresa/:periodo', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        (0, logger_1.logError)('Erro ao buscar dados de estoque', error instanceof Error ? error : new Error('Unknown error'));
        res.status(constants_1.HTTP_STATUS.INTERNAL_SERVER_ERROR).json({
            success: false,
            error: constants_1.MESSAGES.ERROR.INTERNAL_ERROR,
            message: 'Erro interno do servidor',
...
router.get('/ciap/:empresa/:periodo', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        (0, logger_1.logError)('Erro ao buscar dados de CIAP', error instanceof Error ? error : new Error('Unknown error'));
        res.status(constants_1.HTTP_STATUS.INTERNAL_SERVER_ERROR).json({
            success: false,
            error: constants_1.MESSAGES.ERROR.INTERNAL_ERROR,
            message: 'Erro interno do servidor',
...
router.get('/relatorio/:empresa/:periodo', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        (0, logger_1.logError)('Erro ao gerar relatório de estoque e CIAP', error instanceof Error ? error : new Error('Unknown error'));
        res.status(constants_1.HTTP_STATUS.INTERNAL_SERVER_ERROR).json({
            success: false,
            error: constants_1.MESSAGES.ERROR.INTERNAL_ERROR,
            message: 'Erro interno do servidor',
