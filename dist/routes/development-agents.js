...
router.post('/quality', async (req, res) => {
    try {
        ...
        const { CodeQualityAgent } = await Promise.resolve().then(() => __importStar(require('../services/agents/code-quality-agent')));
        const agent = new CodeQualityAgent({
            openaiApiKey: process.env.OPENAI_API_KEY,
            projectPath: process.cwd()
        });
        const result = await agent.runFullQualityAnalysis();
        res.json({
            success: result.success,
            data: result,
