...
router.get('/stats', async (req, res) => {
    try {
        ...
        const calculosIcms = await cache.get('calculos:icms:count') || 0;
        const valorTotalIcms = await cache.get('calculos:icms:total') || 0;
        const calculosProtege = await cache.get('calculos:protege:count') || 0;
        const valorTotalProtege = await cache.get('calculos:protege:total') || 0;
        const stats = {
