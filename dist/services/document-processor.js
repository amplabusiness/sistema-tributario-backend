...
class DocumentProcessor {
    ...
    static async saveSpedContribuicoesData(documentId, empresaId, itens, apuracao) {
        for (const item of itens) {
            await prisma_1.default.spedContribuicoesItem.create({
                data: {
                    documentId,
                    empresaId,
        ...
        for (const ap of apuracao) {
            await prisma_1.default.spedContribuicoesApuracao.create({
                data: {
                    documentId,
                    empresaId,
                    tipo: ap.tipo,
    ...
    static async listDocuments() {
        try {
            const documentos = await prisma_1.default.document.findMany({
                include: {
                    empresa: true,
                },
